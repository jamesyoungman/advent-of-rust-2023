...
fn part2_holding(cards: &[Card]) -> Vec<usize> {
    fn won(mut holding: Vec<usize>, (card_num, wins): (usize, usize)) -> Vec<usize> {
        //eprintln!("card {} has {wins} wins", card_num + 1);
        for card in (card_num + 1)..(card_num + 1 + wins) {
            holding[card] += holding[card_num];
        }
        holding
    }
    let holding: Vec<usize> = cards.iter().map(|_card| 1).collect();
    cards
        .iter()
...
fn main() {
    ...
    println!("day 04 part 2: {}", part2(&get_input()));
}
